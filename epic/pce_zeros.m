function ret = pce_zeros(pc_space, n_sz1, sz2, sz3)
...
% \brief create array of all deterministic zeros in polynomial chaos expansion (PCE) space
%
% This function may become deprecated if zeros function support is added to pce class.
%
...
% \param sz3 the third and final dimension of the zeroed array; optional
	...
	if(nargin == 1)
		...
		ret = pce(pc_space, 0.0, 0.0, 0);
		
	elseif(nargin == 2)
		
		if( mod(n_sz1, 1) ~= 0)
			
			error('poe_zeros(pc_space, n_sz1) -- n_sz1 must be integer');
			
		else
			
			ret(n_sz1, n_sz1) = pce(pc_space, 0.0, 0.0, 0);
			for i = 1:1:numel(ret)
				ret(i) = pce(pc_space, 0.0, 0.0, 0);
			end
	...
	elseif(nargin == 3)
		...
		if( mod(n_sz1, 1) ~= 0 || mod(sz2, 1) ~= 0)
			...
			error('poe_zeros(pc_space, n_sz1, sz2) -- n_sz1 and sz2 must be integers');
			
		else
			
			ret(n_sz1, sz2) = pce(pc_space, 0.0, 0.0, 0);
			for i = 1:1:numel(ret)
				ret(i) = pce(pc_space, 0.0, 0.0, 0);
			end
	...
	elseif(nargin == 4)
		...
		if( mod(n_sz1, 1) ~= 0 || mod(sz2, 1) ~= 0 || mod(sz3, 1) ~= 0)
			...
			error('poe_zeros(pc_space, n_sz1, sz2, sz3) -- n_sz1, sz2, and sz3 must be integers');
			
		else
			
			ret(n_sz1, sz2, sz3) = pce(pc_space, 0.0, 0.0, 0);
			for i = 1:1:numel(ret)
				ret(i) = pce(pc_space, 0.0, 0.0, 0);
			end
		...
		error('poe_zeros() -- number of arguments must be between 1 and 4');
	end
