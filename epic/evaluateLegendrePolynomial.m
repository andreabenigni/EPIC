function ret = evaluateLegendrePolynomial(x,order)
...
if(~isscalar(order))
    error('evaluateLegendrePolynomial(x,order) -- order must be a scalar');
end %if
if(order < 0)
    error('evaluateLegendrePolynomial(x,order) -- order cannot be less than 0');
end %if
