...
% \brief overloaded operator for per-element multiplication (.*) for pce object arrays

	if(size(a) == size(b))
		
		nelement = numel(a);
		c(size(a,1),size(a,2)) = stimes(a(nelement),b(nelement));
	...
	elseif(isscalar(b))
		...
		error('pce::times(a,b) -- a and b must have same dimensions/size or at least one of a and b must be a scalar');
	end
