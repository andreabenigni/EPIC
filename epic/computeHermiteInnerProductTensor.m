function C = computeHermiteInnerProductTensor(num_variables, order)
    ...
    for j=1:P
        ...
        for k=1:P
            ...
            for s=1:num_variables
                ...
                % Quadrature formula
                for t=1:12
                    integ(s)=integ(s)+c(t)*evaluateHermitePolynomial(x(t),ind(i,s))*evaluateHermitePolynomial(x(t),ind(j,s))*evaluateHermitePolynomial(x(t),ind(k,s));                   
                end
...
for k=1:P
    ...
    % Calculate the inner product dimension by dimension
      for s=1:num_variables
            ...
            % Quadrature formula
            for t=1:12
                integ(s)=integ(s)+c(t)*evaluateHermitePolynomial(x(t),ind(k,s))*evaluateHermitePolynomial(x(t),ind(k,s));
            end
            norm = norm*integ(s);
     end
     C(:,:,k) = C(:,:,k)/norm;
