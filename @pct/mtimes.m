...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if size(a,1)==1 && size(a,2)==1
        ...
        %Check if the inner dimension is the same
        if size(a,2)~=size(b,1)
            error('Inner matrix dimensions must agree. (pct_mtimes)')
        end
        

        %initialize the output c to the right size according to a and b
        %needed otherwise error of converion to double from pct when creating
        ...
        %for the general general case of a matrix multiplication calculate value
        %for each element of c
        %i1 and j1 move along the dimension of c
        %k1 move along the inner dimension
        for i1 = 1:size(a,1)
            for j1 = 1:size(b,2)
                for k1 = 1:size(a,2)
                    c(i1,j1) = c(i1,j1)+mscalar(a(i1,k1),b(k1,j1));
                end
    ...
    %function to perform multiplication of two scalar
